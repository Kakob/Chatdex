...
**Also approved 2026-08-28:** the Change Workspace — `docs/SPEC-change-workspace.md` (PRD: `docs/PRD-code-ownership-loop.md`). Extends Prepare Change into a persistent workspace: intent → evidence (repo search over a local read-only file cache) → trace → hypothesis → implementation (attached, never produced by Chatdex) → verification → learned → promote. Milestones CW-0…CW-8, one per session, logged in `docs/CHANGE-WORKSPACE-BUILD-LOG.md` with Jacob's actions in `docs/CHANGE-WORKSPACE-TODOS.md`; the spec's §2 laws and §15 security audit are binding. Guided mode makes no LLM calls; Assisted mode (CW-7) is disclosed, opt-in, and lands only as `ai_inference`.
...
6. **Synthesis may use user-authenticated LLM providers.** The understanding layer (project discovery, understanding synthesis — PRD stages U1+) may send conversation plaintext to LLM providers, under these conditions:
   ...
   - **Repository excerpts follow the same rule (Intent Trace, 2026-08-28).** File and spec-document excerpts fetched read-only from GitHub are user data and may be sent to the provider under the same user-initiated, disclosed, transit-only conditions — the disclosure must say repository excerpts are being sent. The GitHub token is device-local, goes only to api.github.com, and never enters a relay body or a prompt (`docs/SPEC-intent-trace.md` §13). **Whole repository files may be cached locally for Change Workspace search** (GitHub at a pinned sha, or a user-picked local directory, read-only) in a LOCAL-ONLY IndexedDB table that is never synced; synced workspace records carry only paths, shas, line ranges, and capped quotes (`docs/SPEC-change-workspace.md` §2.5).
